...
# ----- Variant-specific preflight: PRIMARY KEY requirement for resumable -----
#
# The resumable variant uses sqldata's IGNORE-on-duplicate semantics to make
# batch resume idempotent: if a batch fails midway, re-running the batch lets
# IGNORE silently skip rows already committed in the partial run. This only
# works when the storage engine can identify a duplicate, which requires a
# PRIMARY KEY (PK columns are NOT NULL by definition, so PK-based dedup is
# never ambiguous about which row is "the same row").
#
# A table without a PRIMARY KEY would produce real duplicate rows on resume,
# silently corrupting the migrated data. So we hard-fail here.
#
# single_pass mode doesn't resume — a failure means re-running from scratch —
# so the PK requirement doesn't apply and this check is skipped.
#
# When TWO_STEP_VARIANT is unset or empty (e.g. the preflight script is being
# run standalone outside the launcher), default to single_pass behavior so
# existing workflows aren't broken by this new check.
TWO_STEP_VARIANT="${TWO_STEP_VARIANT:-single_pass}"

case "$TWO_STEP_VARIANT" in
  single_pass)
    : # No PK check. Maintain quiet behavior for the default path.
    ;;
  resumable)
    echo "==> Variant 'resumable': checking PRIMARY KEY on all source tables..."

    if [[ -n "$SRC_DBS" ]]; then
      IFS=',' read -r -a PK_DB_LIST <<< "$SRC_DBS"
    else
      PK_DB_LIST=("$SRC_DB")
    fi

    # Build an IN (...) list of schema names for the information_schema query.
    # Tab is set as the field separator below; the script that consumes this
    # query expects two TAB-separated columns: schema, table.
    pk_in_list=""
    for db in "${PK_DB_LIST[@]}"; do
      db="${db// /}"
      [[ -z "$db" ]] && continue
      # Escape single quotes for SQL string literal safety.
      db_esc="${db//\'/\'\'}"
      if [[ -n "$pk_in_list" ]]; then
        pk_in_list="${pk_in_list},'${db_esc}'"
      else
        pk_in_list="'${db_esc}'"
      fi
    done

    if [[ -z "$pk_in_list" ]]; then
      echo "ERROR: resumable variant: no source databases resolved from SRC_DB/SRC_DBS"
      exit 9
    fi

    # The query lists every BASE TABLE in the requested schemas that has no
    # entry in information_schema.STATISTICS with INDEX_NAME='PRIMARY'.
    # Views, sequences, and other non-base objects are excluded — they have
    # no rows and aren't subject to the resume-duplication concern.
    pk_query="SELECT t.TABLE_SCHEMA, t.TABLE_NAME
FROM information_schema.TABLES t
WHERE t.TABLE_SCHEMA IN (${pk_in_list})
...
ORDER BY t.TABLE_SCHEMA, t.TABLE_NAME;"
    ...
    # Use the migration user (SRC_USER) — the same identity that sqldata will
    # use at load time. If SRC_USER lacks SELECT on information_schema for
    # these schemas, the PK check fails closed, surfacing the privilege gap
    # now instead of at load time when it would be more disruptive.
    pk_missing="$(MYSQL_PWD="$SRC_PASS" "$MYSQL_BIN" "${src_args[@]}" -u"$SRC_USER" \
      -e "$pk_query" 2>/dev/null || true)"

    if [[ -n "$pk_missing" ]]; then
      echo
      echo "ERROR: The resumable variant requires a PRIMARY KEY on every migrated table."
      echo "       Without one, a failed batch would re-insert duplicate rows on resume."
      echo
      echo "       The following tables don't have a PRIMARY KEY:"
      echo
      # The query output is TAB-separated (schema<TAB>table). Sort for
      # deterministic ordering across runs, then format as indented
      # "schema.table" lines.
      printf '%s\n' "$pk_missing" \
        | sort \
        | awk -F'\t' '{ printf "         %s.%s\n", $1, $2 }'
      ...
      echo "         1. Add a PRIMARY KEY to each table, or"
      echo "         2. Re-run and choose 'single_pass' (no resume, no PK requirement)"
      echo
      exit 10
    fi
    echo "    All tables have a PRIMARY KEY (OK)."
    ;;
  *)
    echo "ERROR: TWO_STEP_VARIANT='${TWO_STEP_VARIANT}' is not recognized."
    echo "       Expected 'single_pass' or 'resumable'."
    exit 11
    ;;
esac
